...
import { dashboardApi, ordersApi, customersApi, getBaseUrl } from '../utils/api';
...
interface DashboardContextType {
  ...
  setExternalCompanyId: (id: string) => void;
...
export function DashboardProvider({ children }: { children: ReactNode }) {
  ...
  const [externalCompanyId, setExternalCompanyId] = useState<string>('All');
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [statsRes, ordersRes, custRes] = await Promise.all([
        ...
        ordersApi.getAll(undefined, deliveryType === 'All' ? undefined : deliveryType, externalCompanyId === 'All' ? undefined : externalCompanyId),
        customersApi.getAll(),
      ]);
  ...
  useEffect(() => {
    ...
    const connection = new signalR.HubConnectionBuilder()
      .withUrl(`${getBaseUrl()}/api/orderHub`, {
    ...
    const startConnection = async () => {
      try {
        ...
        connection.onreconnected(() => {
          if (isMounted) setIsDisconnected(false);
        });
  ...
  useEffect(() => {
    if (mounted && authData.token) {
      fetchData();
    }
  }, [mounted, authData.token, authData.tenantId, deliveryType, externalCompanyId]);
