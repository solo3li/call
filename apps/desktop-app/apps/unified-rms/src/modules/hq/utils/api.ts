...
export const getBaseUrl = () => {
  ...
  // Use the current window origin so the HQ app sends requests to the right domain
  // e.g. hq.167.71.66.188.nip.io/api/... instead of 167.71.66.188.nip.io/api/...
  if (typeof window !== 'undefined') {
    return window.location.origin;
  }
  return 'https://hq.167.71.66.188.nip.io';
};
...
export const ordersApi = {
  ...
  updateItemStatus: (orderId: string, itemId: number, status: string) => 
    api.put<any>(`/orders/${orderId}/items/${itemId}/status`, { status }),
};
...
export const kitchenStationsApi = {
  getAll: () => api.get<import('../types/api').KitchenStation[]>('/KitchenStations'),
  create: (data: Partial<import('../types/api').KitchenStation>) => api.post<import('../types/api').KitchenStation>('/KitchenStations', data),
  update: (id: number, data: Partial<import('../types/api').KitchenStation>) => api.put<import('../types/api').KitchenStation>(`/KitchenStations/${id}`, data),
  delete: (id: number) => api.delete(`/KitchenStations/${id}`),
};
...
export const externalCompaniesApi = {
  getAll: () => api.get<any[]>('/externalcompanies'),
};

export const callRecordsApi = {
  getAll: (params?: { tenantId?: string; handledBy?: string; page?: number; pageSize?: number }) =>
    api.get<{ total: number; page: number; pageSize: number; records: any[] }>('/hq/call-records', { params }),
};
