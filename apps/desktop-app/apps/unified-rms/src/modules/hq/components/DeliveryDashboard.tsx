...
export default function DeliveryDashboard() {
 ...
 globalStatus = { text: "ضغط توصيل", color: "bg-carbon-warning text-white", icon: <Activity size={14} /> };
 }
 ...
 <div className="bg-brand-blue text-white px-2 py-1 border-carbon-border flex gap-1 items-center">
 <Truck size={10} /> طلبات نشطة: {deliveryOrders.length}
 </div>
 <div className="bg-brand-green text-white px-2 py-1 border-carbon-border flex gap-1 items-center">
 <User size={10} /> متاح: {availableDrivers.length}
 </div>
 <div className="bg-carbon-warning text-white px-2 py-1 border-carbon-border flex gap-1 items-center">
 <MapPin size={10} /> في مهمة: {busyDrivers.length}
 </div>
 <div className={`${avgWaitTime > 15 ? 'bg-brand-red text-white animate-pulse' : 'bg-carbon-layerHover text-carbon-text'} px-2 py-1 border-carbon-border flex gap-1 items-center`}>
 ...
 <select 
 className="border-carbon-border rounded-none px-2 py-1 text-[11px] font-bold focus:outline-none focus:bg-carbon-warning"
 value={deliveryType}
 onChange={(e) => {
 ...
 <select
 className="border-carbon-border rounded-none px-2 py-1 text-[11px] font-bold focus:outline-none focus:bg-carbon-warning disabled:opacity-50 disabled:bg-carbon-bg"
 ...
 <span className="text-[10px] font-bold text-carbon-textSecondary bg-carbon-bg px-1 border-carbon-border">{mins}د</span>
 </div>
 <p className="text-[10px] font-bold text-carbon-textSecondary truncate pl-6">{order.customerName}</p>
 {order.isExternalDelivery ? (
 <p className="text-[9px] font-bold text-brand-orange bg-carbon-warning/10 px-1 mt-0.5 inline-block border-carbon-blue/30">
 توصيل خارجي{order.externalCompanyName ? ` - ${order.externalCompanyName}` : ''}
 </p>
 ) : (
 <p className="text-[9px] font-bold text-brand-green bg-brand-green/10 px-1 mt-0.5 inline-block border-carbon-success/30">
 توصيل داخلي
 </p>
 )}
 {!order.externalCompanyName && order.address && <p className="text-[9px] font-bold text-carbon-textSecondary truncate pl-6 mt-0.5">{order.address}</p>}
 </motion.div>
 );
 })}
 </AnimatePresence>
 ...
 <div className="flex-1 bg-carbon-warning/10 border-carbon-warning/50 p-1.5 min-w-[200px] flex flex-col gap-2 relative">
 <div className="sticky top-0 z-10 bg-carbon-warning text-carbon-text border-b-2 border-carbon-warning py-1 text-center shadow-sm">
 ...
 let cardBg = isCritical ? 'bg-brand-red text-white border-carbon-border shadow-sm' : 'bg-carbon-layer border-carbon-blue shadow-[2px_2px_0px_#F97316]';
 ...
 {order.address && <p className={`text-[9px] font-bold mt-0.5 truncate ${isCritical ? 'text-gray-300' : 'text-carbon-textSecondary'}`}>{order.address}</p>}
 </div>
 </div>
 <span className={`text-[10px] font-bold px-1.5 py-0.5 border flex items-center gap-0.5 shrink-0 ${isCritical ? 'bg-black border-carbon-border text-white animate-pulse' : 'bg-carbon-warning border-carbon-blue text-white'}`}>
 ...
 <div className="mt-2">
 {order.isExternalDelivery ? (
 <div className="w-full bg-carbon-warning text-white py-1.5 text-[11px] font-bold shadow-sm flex justify-center items-center gap-1 uppercase">
 توصيل خارجي{order.externalCompanyName ? ` - ${order.externalCompanyName}` : ''}
 </div>
 ...
 <button onClick={() => setAssigningOrderId(null)} className="w-full text-center bg-carbon-bg text-[10px] font-bold py-1.5 mt-1 hover:bg-carbon-layerHover">إلغاء</button>
 </div>
 ) : (
 <button 
 onClick={() => setAssigningOrderId(order.id)}
 className="w-full bg-brand-blue text-white py-1.5 text-[11px] font-bold border-carbon-border shadow-sm hover:bg-carbon-warning hover:text-carbon-text active:translate-y-px active:shadow-none transition-all flex justify-center items-center gap-1 uppercase"
 >
 <Truck size={12} /> تعيين
 </button>
 )}
 ...
 <div className="flex-1 bg-brand-blue/5 border-brand-blue/30 p-1.5 min-w-[200px] flex flex-col gap-2 relative">
 ...
 exit={{ opacity: 0, scale: 0.9 }}
 key={order.id} 
 className="bg-brand-blue text-white border-carbon-border p-2 shadow-sm"
 >
 ...
 {order.driverPhone && <p className="text-[8px] font-bold text-carbon-textSecondary">{order.driverPhone}</p>}
 </div>
 </div>
 </motion.div>
 ))}
 ...
 {/* Column 4: Completed */}
 <div className="flex-1 bg-brand-green/5 border-carbon-success/30 p-1.5 min-w-[200px] flex flex-col gap-2 relative">
 <div className="sticky top-0 z-10 bg-brand-green text-white border-b-2 border-carbon-success py-1 text-center shadow-sm">
 <h2 className="font-bold text-[11px] flex items-center justify-center gap-1">
 ...
 exit={{ opacity: 0, scale: 0.9 }}
 key={order.id} 
 className="bg-brand-green/10 text-brand-green border-carbon-success/50 p-2 shadow-sm opacity-80"
 >
 <div className="flex justify-between items-center mb-1">
 <span className="font-bold text-base leading-none">#{order.orderNumber}</span>
 ...
 {order.address && <p className="text-[9px] font-bold text-carbon-textSecondary truncate mt-0.5">{order.address}</p>}
 {order.driverName && (
 ...
 <h2 className="text-[11px] font-bold mb-2 flex items-center justify-between border-b border-carbon-border pb-1">
 ...
 <div className="flex items-center justify-between text-[10px] font-bold text-carbon-text">
 <span className="truncate">{d.fullName}</span>
 <span className="text-[8px] bg-carbon-warning text-white px-1">{myOrders.length} طلب</span>
 </div>
 <div className="flex gap-0.5 flex-wrap">
 {myOrders.map(mo => (
 <span key={mo.id} className="text-[8px] font-bold text-carbon-textSecondary">#{mo.orderNumber}</span>
 ))}
 </div>
 </div>
 );
 ...
 className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black text-white p-3 border-carbon-warning shadow-[8px_8px_0px_#1A1A1A] z-50 flex flex-col sm:flex-row items-center gap-4 max-w-2xl w-[90%]"
 >
 <div className="flex items-center gap-3 shrink-0">
 <span className="bg-carbon-warning text-black px-3 py-1 text-2xl font-bold">{selectedOrderIds.length}</span>
 <span className="font-bold text-lg uppercase tracking-wider">طلبات للتعيين</span>
 </div>
 
 <div className="flex-grow w-full">
 <select 
 className="w-full bg-carbon-layer text-black border-white p-2 text-lg font-bold cursor-pointer focus:outline-none focus:border-carbon-warning transition-colors"
 onChange={(e) => {
 ...
 {assignLoading && <RefreshCw className="animate-spin text-carbon-warning shrink-0" size={24} />}
 </motion.div>
 )}
 </AnimatePresence>
 </Box>
