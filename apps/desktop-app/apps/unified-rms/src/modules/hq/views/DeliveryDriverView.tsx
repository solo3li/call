...
export default function DeliveryDriverView() {
  ...
  if (!isLoggedIn) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-4">
        <div className="bg-carbon-layer border border-carbon-border bg-carbon-layer w-full max-w-sm p-6 text-center">
          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center border border-carbon-border mx-auto mb-4 ">
          ...
          <form onSubmit={handleManualLogin} className="space-y-4 mt-2">
            <input
              ...
              className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full text-center text-lg font-semibold dir-ltr"
              required
            />
            <button type="submit" className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover w-full bg-[#edf5ff] text-[#0f62fe] text-white py-3 text-lg">
              دخول
            </button>
          </form>
        </div>
  ...
  return (
    <div className="max-w-md mx-auto space-y-4 pb-20">
      {/* Header */}
      <div className="bg-carbon-layer border border-carbon-border bg-[#edf5ff] text-[#0f62fe] text-white p-4 flex items-center justify-between">
        <div>
        ...
        <button onClick={handleLogout} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-layer text-carbon-text border border-carbon-border hover:bg-carbon-layerHover text-carbon-error px-3 py-1.5 text-xs">
          خروج
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-carbon-layer border border-carbon-border bg-carbon-bg text-center py-12 border-dashed">
          <Truck size={48} className="mx-auto text-gray-300 mb-4" />
          <p className="font-semibold text-carbon-textSecondary">لا توجد رحلات مسندة إليك حالياً</p>
        </div>
      ) : (
        <div className="space-y-4">
          <div className="bg-carbon-layer border border-carbon-border bg-carbon-layer/30 p-4 border-2 border-brand-yellow">
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-semibold text-xl">{activeTrip.tripNumber}</h3>
              <span className="px-2 py-1 text-xs font-medium bg-white">{activeTrip.orders.length} طلبات</span>
            </div>
            {activeTrip.mapsUrl && (
              <a href={activeTrip.mapsUrl} target="_blank" rel="noreferrer" className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover w-full bg-[#edf5ff] text-[#0f62fe] text-white py-3 flex items-center justify-center gap-2 font-semibold">
          ...
          {activeTrip.orders.map((order: SuggestedOrderItem, index: number) => {
            ...
            return (
              <div key={order.id} className={`bg-carbon-layer border border-carbon-border p-0 overflow-hidden ${isCompleted ? 'opacity-60' : ''}`}>
                <div className="bg-gray-100 p-3 border-b-2 border-carbon-border flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <span className="w-6 h-6 bg-[#edf5ff] text-[#0f62fe] text-white rounded-full flex items-center justify-center font-semibold text-xs">
                      {index + 1}
                    </span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-1 text-xs font-medium bg-white">{order.totalAmount} {currencySymbol}</span>
                  </div>
                </div>
                  ...
                  <div className="p-3 border-t-2 border-carbon-border bg-carbon-bg space-y-2">
                    {confirmingOrderId === order.id ? (
                      <div className="flex gap-2">
                        <button onClick={() => setConfirmingOrderId(null)} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover flex-1 bg-white py-2 text-sm">إلغاء</button>
                        <button onClick={() => completeOrder(order.id)} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover flex-[2] bg-[#defbe6] text-[#198038] text-white py-2 flex items-center justify-center gap-1 text-sm font-semibold">
                          تأكيد الاستلام والتوصيل
                        </button>
                      </div>
                    ) : (
                      <div className="flex gap-2">
                        <button onClick={() => openGoogleMaps(order.customerAddress)} disabled={!order.customerAddress} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover flex-1 bg-white text-carbon-blue disabled:opacity-40">
                          <Navigation size={18} className="mx-auto" />
                        </button>
                        <button onClick={() => setConfirmingOrderId(order.id)} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover flex-[3] bg-[#defbe6] text-[#198038] text-white font-semibold py-2">
                          تم توصيل هذا الطلب
                        </button>
                      </div>
                    )}
