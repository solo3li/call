...
export function BranchesPage() {
  ...
  return (
    <div className="space-y-6">
      <div className={`bg-[#edf5ff] text-[#0f62fe] bg-carbon-layer border border-carbon-border p-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between`}>
        ...
        <button onClick={() => setShowAddForm(!showAddForm)} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-layer text-carbon-text border border-carbon-border hover:bg-carbon-layerHover px-5 py-2.5 flex items-center justify-center gap-2">
          <Plus size={18} />
          <span>فرع جديد</span>
        </button>
      </div>
      ...
      {showAddForm && (
        <form onSubmit={handleAdd} className="bg-carbon-layer border border-carbon-border p-5 bg-[#FFFBEB] flex flex-col gap-4 animate-fade-in">
          <h3 className="font-semibold text-lg">إضافة فرع جديد</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <input required type="text" placeholder="اسم الفرع" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" value={newBranch.name} onChange={e => setNewBranch({...newBranch, name: e.target.value})} />
            <input required type="text" placeholder="العنوان" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" value={newBranch.address} onChange={e => setNewBranch({...newBranch, address: e.target.value})} />
            <select className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" value={newBranch.status} onChange={e => setNewBranch({...newBranch, status: e.target.value})}>
              <option value="Open">مفتوح</option>
              <option value="Closed">مغلق</option>
            </select>
          </div>
          <button type="submit" className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover py-3 mt-2">حفظ الفرع</button>
        </form>
      )}
      ...
      {shareBranch && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShareBranch(null)}>
          <div 
            ...
            className="bg-white bg-carbon-layer border border-carbon-border p-8 max-w-sm w-full text-center space-y-6"
            ...
            <div className="bg-slate-50 p-6 rounded-sm flex flex-col items-center gap-4 border border-carbon-border">
            ...
            <div className="space-y-3">
              <button 
                ...
                className="w-full px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover py-3 flex items-center justify-center gap-2">
                <Share2 size={18} />
              ...
              <a 
                ...
                target="_blank" 
                className="w-full px-4 py-2 text-sm font-medium transition-colors bg-carbon-layer text-carbon-text border border-carbon-border hover:bg-carbon-layerHover py-3 flex items-center justify-center gap-2">
      ...
      <div className="bg-carbon-layer border border-carbon-border bg-white p-0 overflow-hidden overflow-x-auto border border-carbon-border rounded-none ">
        <table className="w-full text-right border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-[#edf5ff] text-[#0f62fe]/10 border-b-2 border-carbon-border text-[10px]">
          ...
          <tbody className="divide-y divide-neo-border bg-white text-xs">
            {displayBranches.length === 0 ? (
              ...
              displayBranches.map(branch => (
                <tr key={branch.id} className="hover:bg-[#edf5ff] text-[#0f62fe]/5 transition-colors">
                  ...
                  <td className="border-l border-carbon-border px-2 py-1.5 align-top">
                    <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-none border border-carbon-border shadow-sm block w-max ${branch.status === "Open" ? "bg-[#defbe6] text-[#198038] text-white" : "bg-carbon-error text-white"}`}>
                  ...
                  <td className="px-2 py-1.5 align-top text-center">
                    <div className="flex gap-1 justify-center">
                      <button onClick={() => setShareBranch(branch)} className="p-1 bg-carbon-layer text-carbon-text border border-carbon-border shadow-sm hover:translate-y-px transition-all" title="مشاركة المنيو">
                        <QrCode size={12} strokeWidth={3} />
                      </button>
                      <button onClick={() => handleDelete(branch.id)} className="p-1 bg-carbon-error text-white border border-carbon-border shadow-sm hover:translate-y-px transition-all" title="حذف الفرع">
...
function ZoneMapModal({ onSave, onClose }: { onSave: (coords: string) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70">
      <div className="bg-white border border-carbon-border  rounded-sm w-full max-w-3xl mx-4 overflow-hidden flex flex-col" style={{ maxHeight: '90vh' }}>
        {/* Header */}
        <div className="bg-carbon-layer p-4 border-b-4 border-carbon-border flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="px-2 py-1 text-xs font-medium bg-[#fcf4d6] text-[#b47a00] text-white">
        ...
        {/* Search Bar */}
        <form onSubmit={handleSearch} className="bg-white p-2 border-b-4 border-carbon-border flex gap-2">
          <input 
            type="text" 
            placeholder="ابحث عن مدينة، حي، أو شارع..." 
            className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary flex-1 py-2 px-3 text-sm font-medium border border-carbon-border"
            value={searchQuery}
          ...
          <button 
            ...
            className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover text-white px-4 py-2 text-sm font-semibold disabled:opacity-50"
          >
        ...
        <div className="p-4 border-t-4 border-carbon-border bg-carbon-bg flex gap-2 justify-between">
          <button
            type="button"
            onClick={clearAll}
            className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-layer text-carbon-text border border-carbon-border hover:bg-carbon-layerHover text-carbon-error border border-carbon-border px-4 py-2 text-sm font-semibold"
          >
            مسح الكل
          </button>
          <div className="flex gap-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-layer text-carbon-text border border-carbon-border hover:bg-carbon-layerHover border border-carbon-border px-4 py-2 text-sm font-semibold"
            >
              إغلاق
            </button>
            <button
              type="button"
              onClick={handleSave}
              className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover text-white border border-carbon-border px-4 py-2 text-sm font-semibold"
            >
              حفظ المنطقة 
            </button>
          </div>
...
function DeliveryZoneManager({ branchId }: { branchId: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-[#FFFBEB] p-4 border border-carbon-border  space-y-4">
        <form onSubmit={handleAdd} className="space-y-3">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <input required type="text" placeholder="اسم المنطقة" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary flex-1 py-2 px-3 text-sm font-medium" value={newZone.name} onChange={e => setNewZone({...newZone, name: e.target.value})} />
            <input required type="number" placeholder="رسوم التوصيل" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full sm:w-32 py-2 px-3 text-sm font-medium" value={newZone.deliveryCost || ''} onChange={e => setNewZone({...newZone, deliveryCost: parseFloat(e.target.value) || 0})} />
          </div>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover border border-carbon-border px-4 py-2 text-sm font-semibold  hover:-translate-y-[1px]"
              >
                تحديد على الخريطة 🗺️
              </button>
              {newZone.coordinates && (
                <span className="text-xs font-semibold text-carbon-success bg-[#defbe6] text-[#198038]/10 px-2 py-1 border border-brand-green/30">
                   تم تحديد {getCoordCount(newZone.coordinates)} نقطة
                </span>
              )}
            </div>
            <button type="submit" className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover text-white px-6 py-2 text-sm font-semibold border border-carbon-border  hover:-translate-y-[1px]">حفظ المنطقة</button>
          </div>
        </form>

        {zones.length > 0 && (
          <div className="pt-3 border-t-2 border-dashed border-carbon-border">
            <input 
              type="text" 
              placeholder=" ابحث عن منطقة توصيل..." 
              className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full py-2 px-3 text-sm font-medium border border-carbon-border"
      ...
      <div className="space-y-2">
        ...
        {!loading && filteredZones.length > 0 && (
          <div className="flex items-center gap-3 px-4 py-2 bg-carbon-layer border border-carbon-border ">
        ...
        {!loading && filteredZones.length === 0 && (
          <div className="flex flex-col items-center justify-center py-10 gap-3 border-2 border-dashed border-carbon-border bg-white/50">
        ...
        {filteredZones.map((z, idx) => (
          <div
            key={z.id}
            className="flex items-center gap-3 bg-white border border-carbon-border  px-4 py-3 hover:-translate-y-[1px] hover: transition-all group"
          >
            {/* Row Number */}
            <span className="w-7 h-7 rounded-full bg-[#fcf4d6] text-[#b47a00]/10 border-2 border-brand-orange flex items-center justify-center text-[11px] font-semibold text-carbon-warning shrink-0">
              {idx + 1}
            </span>

            {/* Zone Icon */}
            <div className="w-9 h-9 bg-[#FFFBEB] border border-carbon-border flex items-center justify-center shrink-0">
            ...
            {/* Coordinates pill */}
            <span className={`hidden sm:inline-flex items-center gap-1 text-[11px] font-semibold w-24 justify-center shrink-0 ${z.coordinates ? 'text-carbon-blue bg-[#edf5ff] text-[#0f62fe]/10 border border-brand-blue/30 px-2 py-0.5 rounded-full' : 'text-gray-300'}`}>
              {z.coordinates ? <>🗺️ {getCoordCount(z.coordinates)} نقطة</> : '—'}
            </span>

            {/* Cost Badge */}
            <span className="font-semibold text-sm text-white bg-[#defbe6] text-[#198038] border border-carbon-border  px-3 py-1 shrink-0 w-20 text-center">
            ...
            <button
              ...
              className="w-8 h-8 flex items-center justify-center bg-red-50 border border-carbon-border text-carbon-error hover:bg-carbon-error hover:text-white  hover:shadow-none transition-all shrink-0"
              title="حذف المنطقة"
            >
...
export function KitchenStationsPage() {
  ...
  return (
    <div className="space-y-6">
      <div className={`bg-carbon-error text-white bg-carbon-layer border border-carbon-border p-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between`}>
        <div className="flex items-center gap-4">
          <div className="bg-carbon-layer border border-carbon-border-flat bg-white text-carbon-error p-3">
            <ChefHat size={28} />
          </div>
          <div>
        ...
        <button onClick={() => { setShowAddForm(!showAddForm); setEditingStation(null); }} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-layer text-carbon-text border border-carbon-border hover:bg-carbon-layerHover text-carbon-error px-5 py-2.5 flex items-center justify-center gap-2">
          <Plus size={18} />
      ...
      {(showAddForm || editingStation) && (
        <form onSubmit={handleSave} className="bg-carbon-layer border border-carbon-border p-5 bg-[#FFFBEB] flex flex-col gap-4 animate-fade-in">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <input required type="text" placeholder="اسم المحطة (مثال: شواية، بيتزا، مشروبات)" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" 
            ...
            <select className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" 
      ...
      <div className="bg-carbon-layer border border-carbon-border p-0 bg-white overflow-hidden border border-carbon-border ">
                ...
                return (
                  <tr key={station.id} className="hover:bg-carbon-error/5 transition-colors">
                    ...
                    <td className="px-1 py-1.5 align-middle text-center">
                      <div className="flex gap-1 justify-center">
                        <button onClick={() => setEditingStation(station)} className="p-1 bg-carbon-layer text-carbon-text border border-carbon-border shadow-sm hover:translate-y-px transition-all">
                          <Edit3 size={12} strokeWidth={3} />
                        </button>
                        <button onClick={() => handleDelete(station.id)} className="p-1 bg-carbon-error text-white border border-carbon-border shadow-sm hover:translate-y-px transition-all">
                          <Trash2 size={12} strokeWidth={3} />
                        </button>
                      </div>
                    </td>
...
export function MenuPage() {
  ...
  return (
    <div className="space-y-6">
      <div className={`bg-[#defbe6] text-[#198038] bg-carbon-layer border border-carbon-border p-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between`}>
        <div className="flex items-center gap-4">
          <div className="bg-carbon-layer border border-carbon-border-flat bg-white p-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowCatForm(!showCatForm)} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover px-5 py-2.5 flex items-center justify-center gap-2">
            <Tag size={18} />
            <span>إضافة فئة</span>
          </button>
          <button onClick={() => setShowAddForm(!showAddForm)} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-layer text-carbon-text border border-carbon-border hover:bg-carbon-layerHover px-5 py-2.5 flex items-center justify-center gap-2">
      ...
      {(showCatForm || editingCat) && (
        <form onSubmit={handleAddCategory} className="bg-carbon-layer border border-carbon-border p-5 bg-carbon-layer/10 flex flex-col gap-4 animate-fade-in border-brand-yellow">
          <div className="flex justify-between items-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <input required type="text" placeholder="اسم الفئة (مثلاً: برجر، بيتزا)" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" 
            ...
            <input required type="text" placeholder="أيقونة الفئة (ايموجي)" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary text-center text-2xl" 
            ...
            <div className="flex flex-col gap-1">
              {(editingCat ? editingCat.imageUrl : newCat.imageUrl) && (
                <div className="w-20 h-20 rounded-sm border border-carbon-border overflow-hidden mb-2 ">
                  <img src={getImageUrl(editingCat ? editingCat.imageUrl : newCat.imageUrl)} className="w-full h-full object-cover" alt="Preview" />
                </div>
              )}
              <input type="text" placeholder="رابط صورة الفئة" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" 
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <input type="file" accept="image/*" onChange={handleCatImageUpload} className="text-xs file:mr-2 file:py-1 file:px-2 file:rounded-sm file:border-2 file:border-carbon-border file:bg-carbon-layer file:text-xs file:font-semibold file: cursor-pointer" />
                {catUploading && <span className="text-xs text-[#f1c21b] font-medium animate-pulse">جاري الرفع...</span>}
              </div>
            </div>
          </div>
          <button type="submit" disabled={catUploading} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover py-3 mt-2 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {(showAddForm || editingItem) && (
        <form onSubmit={handleAddItem} className="bg-carbon-layer border border-carbon-border p-5 bg-[#FFFBEB] flex flex-col gap-4 animate-fade-in">
          <div className="flex justify-between items-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <input required type="text" placeholder="اسم الصنف" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" 
              value={editingItem ? editingItem.name : newItem.name} 
              onChange={e => editingItem ? setEditingItem({...editingItem, name: e.target.value}) : setNewItem({...newItem, name: e.target.value})} />
            <input type="text" placeholder="الوصف" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" 
            ...
            <input required type="number" placeholder="السعر" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" 
            ...
            <div className="flex flex-col gap-1">
              ...
              <input type="text" placeholder="رابط الصورة" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" 
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-xs text-carbon-textSecondary font-medium">أو تحميل:</span>
                <input type="file" accept="image/*" onChange={handleItemImageUpload} className="text-xs file:mr-2 file:py-1 file:px-2 file:rounded-sm file:border-2 file:border-carbon-border file:bg-carbon-layer file:text-xs file:font-semibold file: cursor-pointer" />
                {itemUploading && <span className="text-xs text-[#f1c21b] font-medium animate-pulse">جاري الرفع...</span>}
              </div>
            </div>
            <select required className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary" 
          ...
          <button type="submit" disabled={itemUploading} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover py-3 mt-2 disabled:opacity-50 disabled:cursor-not-allowed">
            {itemUploading ? "جاري رفع الصورة..." : (editingItem ? "تحديث الصنف" : "حفظ الصنف")}
          </button>
        </form>
      )}

      <div className="bg-carbon-layer border border-carbon-border p-4 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between bg-white">
        <div className="flex flex-wrap gap-2">
          <button
            onClick={() => setActiveCategory("الكل")}
            className={`px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover px-4 py-2 text-sm ${activeCategory === "الكل" ? "bg-[#defbe6] text-[#198038]" : "bg-carbon-bg"}`}>
            الكل
          </button>
          {apiCategories.map((cat) => (
            <div key={cat.id} className="relative group">
              <button
                ...
                className={`px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover px-4 py-2 text-sm flex items-center gap-2 ${activeCategory === cat.id ? "bg-[#defbe6] text-[#198038]" : "bg-carbon-bg"}`}>
                {cat.imageUrl ? (
              ...
              <div className="absolute -top-2 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button 
                  ...
                  className="bg-[#edf5ff] text-[#0f62fe] text-white p-1 rounded-full border border-carbon-border  hover:translate-y-[1px]">
                  <Edit3 size={10} />
                </button>
                <button 
                  onClick={() => handleDeleteCategory(cat.id)}
                  className="bg-carbon-error text-white p-1 rounded-full border border-carbon-border  hover:translate-y-[1px]">
                  <X size={10} />
                </button>
              </div>
            </div>
      ...
      <div className="bg-carbon-layer border border-carbon-border p-0 overflow-hidden bg-white border border-carbon-border ">
        <table className="w-full text-right border-collapse">
          <thead>
            <tr className="bg-[#FFFBEB] border-b-2 border-carbon-border text-[10px]">
              ...
              filteredMenu.map((item) => (
                <tr key={item.id} className="hover:bg-carbon-layerHover transition-colors">
                  <td className="px-2 py-1.5 border-l border-carbon-border align-middle text-center">
                    <div className="w-8 h-8 rounded border border-carbon-border bg-white flex items-center justify-center overflow-hidden mx-auto shadow-sm">
                      {item.imageUrl ? (
                  ...
                  <td className="px-2 py-1.5 border-l border-carbon-border align-middle">
                    <span className="text-[9px] font-semibold text-carbon-blue bg-[#edf5ff] text-[#0f62fe]/10 px-1 border border-brand-blue/20">
  ...
  return (
    <div className="space-y-6">
      ...
      {showAddForm && (
        <form onSubmit={handleAdd} className="bg-carbon-layer border border-carbon-border p-6 bg-[#FFFBEB] space-y-6 animate-fade-in border border-carbon-border ">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <input required type="text" placeholder="مثال: أحمد العتيبي" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full" value={newEmployee.fullName} onChange={e => setNewEmployee({...newEmployee, fullName: e.target.value})} />
              </div>
              <div className="space-y-2">
                <label className="font-semibold text-sm">رقم الجوال (للتواصل)</label>
                <div className="relative">
                  <Mail className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                  <input required type="tel" placeholder="05XXXXXXXX" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full pr-12 dir-ltr text-right" value={newEmployee.mobileNumber} onChange={e => setNewEmployee({...newEmployee, mobileNumber: e.target.value})} />
                </div>
              </div>
              <div className="space-y-2">
                <label className="font-semibold text-sm">القسم</label>
                <select required className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full" value={newEmployee.departmentId} onChange={e => setNewEmployee({...newEmployee, departmentId: e.target.value})}>
              ...
              <div className="space-y-2">
                <label className="font-semibold text-sm flex items-center gap-2 cursor-pointer p-4 bg-carbon-bg border-2 border-transparent hover:border-brand-pink/30 rounded-sm transition-all w-full">
              ...
              <div className="space-y-2">
                ...
                <select className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full" value={newEmployee.branchId} onChange={e => setNewEmployee({...newEmployee, branchId: e.target.value})}>
              ...
              <div className="space-y-2">
                ...
                <input type="text" placeholder="EMP123" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full uppercase" value={newEmployee.employeeCode} onChange={e => setNewEmployee({...newEmployee, employeeCode: e.target.value})} />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              {roles.length === 0 && (
                <div className="p-4 bg-gray-100 rounded-sm border-2 border-dashed border-gray-400 text-center">
          ...
          <div className="pt-4 border-t-2 border-carbon-border">
            <button type="submit" disabled={newEmployee.roles.length === 0} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover w-full py-4 text-lg font-semibold flex items-center justify-center gap-2 disabled:opacity-50">
            ...
            {employees.length === 0 ? (
              ...
              employees.map(emp => (
                <tr key={emp.id} className="hover:hover:bg-carbon-layerHover text-carbon-text transition-colors">
                  ...
                  <td className="border-l border-carbon-border px-2 py-1.5 align-middle">
                    <span className={`text-[9px] font-semibold px-1.5 py-0.5 border border-carbon-border shadow-sm block w-max ${emp.status === "Available" ? "bg-[#defbe6] text-[#198038] text-white" : "bg-gray-400 text-white"}`}>
                  ...
                  <td className="px-1 py-1.5 align-middle text-center">
                    <div className="flex items-center justify-center gap-1">
                      <button 
                        ...
                        className="p-1 bg-[#e8daff] text-[#6929c4]/10 text-brand-purple border border-brand-purple/30 shadow-sm hover:translate-y-px transition-all" title="تحديث الرمز">
                        <RefreshCw size={12} strokeWidth={3} />
                      </button>
                      <button 
                        onClick={() => handleDelete(emp.id)}
                        className="p-1 bg-carbon-error/10 text-carbon-error border border-brand-red/30 shadow-sm hover:translate-y-px transition-all" title="حذف الموظف">
                        <Trash2 size={12} strokeWidth={3} />
                      </button>
                    </div>
                  </td>
      ...
      {employees.length === 0 && !loading && (
        <div className="bg-carbon-layer border border-carbon-border p-12 text-center bg-carbon-bg border-dashed border-4 border-gray-300">
...
export function OrdersPage({ onEditOrder }: { onEditOrder?: (id: string) => void }) {
  ...
  if (selectedOrderId) {
    return (
      <div className="space-y-3 animate-fade-in">
        <button onClick={() => setSelectedOrderId(null)} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-layer text-carbon-text border border-carbon-border hover:bg-carbon-layerHover border border-carbon-border px-3 py-1 text-sm font-semibold flex items-center gap-1 hover:bg-carbon-bg transition-colors">
          <ChevronRight size={16} /> عودة للقائمة
        </button>

        {detailsLoading || !orderDetails ? (
           <div className="bg-carbon-layer border border-carbon-border p-10 text-center animate-pulse font-semibold text-xs text-gray-400 bg-white border border-carbon-border">جاري التحميل...</div>
        ) : (
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
             {/* Main Info */}
             <div className="lg:col-span-2 space-y-3">
                <div className="bg-carbon-layer border border-carbon-border bg-white p-3 border border-carbon-border ">
                   <div className="flex justify-between items-center mb-3 pb-2 border-b-2 border-carbon-border">
                      ...
                      <StatusPill 
                        ...
                        color={orderDetails.status === "Pending" ? "bg-carbon-layer" : orderDetails.status === "Preparing" ? "bg-[#fcf4d6] text-[#b47a00] text-white" : orderDetails.status === "Completed" ? "bg-[#defbe6] text-[#198038] text-white" : "bg-carbon-error text-white"} 
                      />
                   </div>

                   {/* Edit Mode Toggle */}
                   {(orderDetails.status === "Pending" || orderDetails.status === "معلق") && !isEditing && (
                      <button onClick={() => onEditOrder && onEditOrder(String(orderDetails.id))} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover w-full bg-carbon-layer border border-carbon-border  py-1.5 mb-3 flex items-center justify-center gap-1 font-semibold text-sm hover:-translate-y-px transition-all">
                        <Edit3 size={14} /> فتح الطلب للتعديل في مركز الاتصال
                      </button>
                    )}

                   {isEditing ? (
                      <div className="space-y-3 bg-[#FFFBEB] p-3 rounded-sm border border-carbon-border border-dashed">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            <div>
                              ...
                              <input type="text" className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full font-medium text-xs py-1 px-2" value={editForm.customerName} onChange={e => setEditForm({...editForm, customerName: e.target.value})} />
                            </div>
                            <div>
                              <label className="block text-[10px] font-semibold text-carbon-text mb-0.5">نوع الطلب</label>
                              <select className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full font-medium text-xs py-1 px-2" value={editForm.orderType} onChange={e => setEditForm({...editForm, orderType: e.target.value})}>
                          ...
                          <div>
                            ...
                            <textarea className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full font-medium text-xs py-1 px-2 min-h-[40px]" value={editForm.kitchenNotes} onChange={e => setEditForm({...editForm, kitchenNotes: e.target.value})} />
                          </div>
                          ...
                          <div className="border-t-2 border-carbon-border pt-3 mt-3">
                            ...
                            <div className="space-y-1 mb-3">
                               {editForm.items.map((item: any) => (
                                  <div key={item.menuItemId} className="flex justify-between items-center bg-white border border-carbon-border p-1.5 rounded-sm">
                                     ...
                                     <div className="flex items-center gap-2">
                                        ...
                                        <div className="flex items-center gap-1 bg-gray-100 p-0.5 rounded border border-carbon-border">
                                          <button onClick={() => updateItemQty(item.menuItemId, -1)} className="w-5 h-5 flex items-center justify-center bg-white rounded font-semibold text-[10px] hover:bg-carbon-error hover:text-white transition-colors">-</button>
                                          <span className="font-semibold w-3 text-[10px] text-center">{item.quantity}</span>
                                          <button onClick={() => updateItemQty(item.menuItemId, 1)} className="w-5 h-5 flex items-center justify-center bg-white rounded font-semibold text-[10px] hover:bg-[#edf5ff] text-[#0f62fe] hover:text-white transition-colors">+</button>
                                        </div>
                                     </div>
                                  </div>
                               ))}
                            ...
                            <div className="bg-white p-2 rounded-sm border border-carbon-border">
                               <p className="font-semibold text-[10px] mb-1">إضافة أصناف جديدة:</p>
                               <input type="text" placeholder="ابحث عن صنف..." className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full mb-1 text-[10px] py-1 px-2 h-7" value={searchItem} onChange={e => setSearchItem(e.target.value)} />
                               <div className="max-h-24 overflow-y-auto custom-scrollbar space-y-0.5">
                                  {filteredMenuItems.slice(0, 10).map(mi => (
                                     <div key={mi.id} className="flex justify-between items-center p-1.5 hover:bg-carbon-layer/10 rounded-sm cursor-pointer border border-transparent hover:border-brand-yellow" onClick={() => updateItemQty(mi.id, 1)}>
                          ...
                          <div className="flex gap-2 pt-2 mt-1">
                            <button onClick={handleSaveEdit} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover flex-1 bg-[#defbe6] text-[#198038] text-white py-1.5 text-xs font-semibold  hover:-translate-y-px transition-transform">حفظ التغييرات </button>
                            <button onClick={() => setIsEditing(false)} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover px-4 bg-white py-1.5 text-xs font-semibold  hover:-translate-y-px transition-transform">إلغاء </button>
                          </div>
                      </div>
                   ) : (
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
                        <div className="bg-carbon-bg p-2 rounded-sm border border-carbon-border">
                        ...
                        <div className="bg-carbon-bg p-2 rounded-sm border border-carbon-border">
                          ...
                          {orderDetails.externalCompanyName && (
                            <span className="block text-carbon-warning bg-[#fcf4d6] text-[#b47a00]/10 mt-1 px-1 py-0.5 text-[9px] border border-brand-orange/30 rounded font-medium uppercase">
                   ...
                   {!isEditing && orderDetails.kitchenNotes && (
                      <div className="mb-3 p-2 bg-carbon-layer/20 rounded-sm border-2 border-brand-yellow">
                     ...
                     <div>
                        ...
                        <div className="space-y-1.5">
                          {orderDetails.items?.map((item: any) => (
                            <div key={item.id} className="flex justify-between items-center bg-carbon-bg border border-carbon-border p-2 rounded-sm ">
                              <div className="flex items-center gap-2">
                                <span className="bg-[#edf5ff] text-[#0f62fe] text-white w-5 h-5 flex items-center justify-center rounded font-semibold text-[10px] border border-carbon-border">{item.quantity}x</span>
             ...
             <div className="lg:col-span-1 space-y-3">
                <div className="bg-carbon-layer border border-carbon-border bg-white p-3 border border-carbon-border ">
                   ...
                   <div className="flex flex-col gap-2">
                      {orderDetails.status === "Pending" && (
                        <button onClick={() => handleStatusChange(String(orderDetails.id), "Preparing")} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover text-white py-1.5 text-xs font-semibold flex items-center justify-center gap-1  hover:-translate-y-px"><Clock size={14}/> البدء بالتحضير</button>
                      )}
                      {orderDetails.status === "Preparing" && (
                        <button onClick={() => handleStatusChange(String(orderDetails.id), "Completed")} className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover text-white py-1.5 text-xs font-semibold flex items-center justify-center gap-1  hover:-translate-y-px"><CheckCircle2 size={14}/> إكمال الطلب</button>
                      )}
                ...
                {orderDetails.audits && orderDetails.audits.length> 0 && (
                  <div className="bg-carbon-layer border border-carbon-border bg-[#FFFBEB] p-3 border border-carbon-border ">
                    ...
                    <div className="space-y-3 relative before:absolute before:inset-0 before:mr-1.5 before:-translate-x-px before:h-full before:w-px before:bg-[#edf5ff] text-[#0f62fe]/30 dir-ltr text-right">
                      {orderDetails.audits.map((audit: any) => (
                        <div key={audit.id} className="relative flex items-start gap-2">
                          <div className="flex items-center justify-center w-4 h-4 rounded-full border border-carbon-border bg-carbon-layer text-carbon-text shrink-0  z-10 mt-1">
                            <div className="w-1 h-1 bg-neo-text rounded-full"></div>
                          </div>
                          <div className="flex-1 bg-white p-2 rounded-sm border border-carbon-border ">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-1 gap-1">
                              <span className="font-semibold text-[10px] text-carbon-blue">{audit.userName}</span>
                              <time className="text-[8px] font-medium text-carbon-textSecondary dir-ltr bg-gray-100 px-1 py-0.5 rounded border border-gray-200">{new Date(audit.timestamp).toLocaleString("ar-SA")}</time>
                            </div>
                            <div className="text-[8px] font-semibold text-white mb-1 inline-block bg-[#fcf4d6] text-[#b47a00] px-1 py-0.5 rounded border border-carbon-border shadow-sm uppercase tracking-wider">{audit.action}</div>
                            <p className="text-[9px] font-medium leading-relaxed text-carbon-textSecondary bg-carbon-bg p-1 rounded border border-gray-200">{audit.changes}</p>
                          </div>
                        </div>
                      ))}
                    </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in relative">
      ...
      <div className="flex flex-col gap-2">
        <div className="bg-white p-2 flex flex-col gap-2 border border-carbon-border ">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input 
                ...
                className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full !pr-8 border border-carbon-border shadow-sm font-medium py-1 text-xs h-8"
              />
            </div>
            ...
            <select 
              ...
              className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary border border-carbon-border font-medium py-1 px-2 h-8 shadow-sm cursor-pointer bg-white text-xs w-auto"
            >
            ...
            <div className="flex items-center gap-1">
              <input
                ...
                className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary border border-carbon-border font-medium py-1 px-1 h-8 shadow-sm text-[10px] cursor-pointer w-32"
              />
            ...
            {(dateFrom || dateTo || quickDate) && (
              <button
                ...
                className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover px-2 py-1 h-8 bg-red-100 border border-carbon-border text-[10px] font-semibold shadow-sm flex items-center gap-1"
              >
                <X size={12} /> مسح
              </button>
            )}
        ...
        <div className="flex gap-1 overflow-x-auto pb-1 custom-scrollbar w-full">
          {["الكل", "Pending", "Preparing", "Delivering", "Completed", "Cancelled"].map(s => (
            <button 
              key={s} 
              onClick={() => setFilter(s)}
              className={`px-3 py-1 text-xs font-semibold border border-carbon-border transition-all flex-1 text-center justify-center whitespace-nowrap ${filter === s ? "bg-carbon-layer " : "bg-white hover:bg-carbon-bg shadow-sm"}`}>
          ...
          <tbody className="divide-y divide-neo-border bg-white text-xs">
            {filteredOrders.length === 0 ? (
              ...
              filteredOrders.map(order => {
                ...
                const rowBg = isDelayed ? "bg-carbon-error text-white" : "hover:bg-carbon-layer/10 bg-white text-carbon-text";
                ...
                <tr key={order.id} className={`${rowBg} transition-colors cursor-pointer border-b border-carbon-border`} onClick={() => fetchOrderDetails(String(order.id))}>
                  ...
                  <td className={`${borderClasses}`}>
                    <span className={`text-[9px] font-semibold px-1 border ${isDelayed ? 'border-white' : 'border-carbon-border bg-gray-100'}`}>
                  ...
                  <td className={`${borderClasses} font-medium text-[10px]`}>
                    {order.externalCompanyName ? (
                      <span className="text-carbon-warning bg-[#fcf4d6] text-[#b47a00]/10 px-1 border border-brand-orange/30 rounded">
                        {order.externalCompanyName}
                      </span>
                    ) : (
                  ...
                  <td className={`${borderClasses} font-medium text-[10px] whitespace-nowrap`}>
                    ...
                    <span className={`mr-1 px-1 text-[9px] font-semibold ${isDelayed ? 'bg-black text-white animate-pulse' : 'bg-gray-200 text-carbon-textSecondary'}`}>{minutesElapsed}د</span>
                  </td>
                  <td className={`${borderClasses}`}>
                    <span className={`text-[9px] font-semibold px-1 border ${isDelayed ? 'border-white' : 'border-carbon-border'} ${order.status === "Pending" ? "bg-carbon-layer text-carbon-text" : order.status === "Preparing" ? "bg-[#fcf4d6] text-[#b47a00] text-white" : order.status === "Delivering" ? "bg-[#e5f6ff] text-[#00a68f] text-carbon-text" : order.status === "Completed" ? "bg-[#defbe6] text-[#198038] text-white" : "bg-carbon-error text-white"}`}>
                  ...
                  <td className="px-1 py-1 align-middle text-center border-l border-carbon-border">
                    <div className="flex gap-1 justify-center">
                        {order.status === "Pending" && (
                          <button onClick={(e) => { e.stopPropagation(); handleStatusChange(String(order.id), "Preparing"); }} className="p-1 bg-[#fcf4d6] text-[#b47a00] text-white border border-carbon-border  hover:translate-y-px transition-all" title="البدء بالتحضير"><Clock size={12} strokeWidth={3}/></button>
                        )}
                        {order.status === "Preparing" && (
                          <button onClick={(e) => { e.stopPropagation(); handleStatusChange(String(order.id), "Completed"); }} className="p-1 bg-[#defbe6] text-[#198038] text-white border border-carbon-border  hover:translate-y-px transition-all" title="إكمال الطلب"><CheckCircle2 size={12} strokeWidth={3}/></button>
                        )}
                        {order.status !== "Completed" && order.status !== "Cancelled" && (
                          <button onClick={(e) => { e.stopPropagation(); handleStatusChange(String(order.id), "Cancelled"); }} className="p-1 bg-carbon-error text-white border border-carbon-border  hover:translate-y-px transition-all" title="إلغاء الطلب"><XCircle size={12} strokeWidth={3}/></button>
                        )}
                    </div>
                  </td>
                </tr>
...
export function ReviewsPage() {
  ...
  return (
    <div className="space-y-6">
      <div className={`bg-[#e5f6ff] text-[#00a68f] bg-carbon-layer border border-carbon-border p-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between`}>
        <div className="flex items-center gap-4">
          <div className="bg-carbon-layer border border-carbon-border-flat bg-white p-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-carbon-layer border border-carbon-border p-5 bg-white lg:col-span-1 flex flex-col items-center justify-center text-center">
        ...
        <div className="bg-carbon-layer border border-carbon-border p-5 bg-white lg:col-span-2">
          <h3 className="font-semibold text-lg mb-4">التقييم اليومي خلال الأسبوع</h3>
          <div className="grid grid-cols-7 gap-2 pt-4">
            {ratings.map((item: any, idx: number) => (
              <div key={idx} className="flex flex-col items-center justify-between bg-[#FFFBEB] p-3 rounded-sm border border-carbon-border  h-32">
                <span className="font-semibold text-xs text-carbon-textSecondary">{item.day}</span>
                <div className="w-full bg-gray-200 h-12 rounded-sm relative overflow-hidden border border-carbon-border">
                  <div 
                    className="absolute bottom-0 left-0 right-0 bg-[#fcf4d6] text-[#b47a00] transition-all" 
      ...
      <div className="bg-carbon-layer border border-carbon-border p-5 bg-white space-y-4">
          ...
          {notificationsList.map((notif: any) => (
            <div key={notif.id} className="py-3 flex items-start justify-between gap-4">
              <div className="flex items-start gap-3">
                <span className="text-2xl p-2 bg-carbon-bg rounded-sm border border-carbon-border ">
...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={handleSave} className="bg-carbon-layer border border-carbon-border p-4 bg-white border border-carbon-border ">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label className="block text-[10px] font-semibold text-carbon-textSecondary mb-1">اسم المطعم</label>
            <input 
              type="text" 
              className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full border border-carbon-border shadow-sm font-medium text-xs py-1 h-8" 
          ...
          <div>
            ...
            <input 
              type="text" 
              disabled
              className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full border border-carbon-border shadow-sm font-medium bg-gray-100 text-left dir-ltr cursor-not-allowed text-xs py-1 h-8" 
        ...
        <div className="mt-4 pt-4 border-t border-carbon-border flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="submit" className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover px-4 py-1.5 flex items-center justify-center gap-1 text-xs font-semibold shadow-sm">
              <Save size={14} />
              <span>حفظ</span>
            </button>
          </div>
      ...
      <div className={`bg-carbon-layer bg-carbon-layer border border-carbon-border p-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between`}>
        <div className="flex items-center gap-4">
          <div className="bg-carbon-layer border border-carbon-border-flat bg-white p-3 text-3xl">
            🤖
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Config Card */}
        <div className="bg-carbon-layer border border-carbon-border p-6 bg-white space-y-6 h-fit">
          <div className="flex items-center justify-between border-b-2 border-carbon-border pb-4">
            ...
            <StatusPill 
              ...
              color={botStatus === "Active" ? "bg-[#defbe6] text-[#198038]" : botStatus === "Error" ? "bg-carbon-error text-white" : "bg-carbon-layer"} 
            />
          </div>
          ...
          <form onSubmit={handleSaveBotConfig} className="space-y-4">
            <div>
              ...
              <input 
                ...
                className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary w-full border border-carbon-border  font-mono text-sm dir-ltr text-left" 
                value={botToken} 
            ...
            {botUsername && (
              <div className="p-3 bg-[#edf5ff] text-[#0f62fe]/10 border border-brand-blue/30 rounded-sm flex items-center justify-between">
            ...
            {botMsg.text && (
              <div className={`p-3 rounded-sm border-2 font-semibold text-xs  animate-fade-in ${
                botMsg.type === "success" ? "bg-[#defbe6] text-[#198038]/20 border-brand-green text-carbon-success" : "bg-carbon-error/20 border-brand-red text-carbon-error"
              }`}>
                {botMsg.text}
              </div>
            )}
            ...
            <div className="flex gap-4 pt-2">
              <button 
                type="submit" 
                disabled={botLoading}
                className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover text-white flex-1 py-3 text-sm font-semibold ">
              ...
              <button 
                ...
                className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover text-carbon-text px-4 py-3 text-sm font-semibold  disabled:opacity-50">
                اختبار الاتصال
              </button>
            </div>
          </form>
        ...
        {/* Simulator Card */}
        <div className="bg-carbon-layer border border-carbon-border p-6 bg-[#FFFBEB] flex flex-col h-[500px]">
          ...
          {/* Chat Messages Area */}
          <div className="flex-1 overflow-y-auto space-y-4 pr-2 flex flex-col border border-carbon-border bg-white rounded-sm p-4 ">
            {simChat.map((msg, idx) => (
              <div key={idx} className={`flex flex-col max-w-[85%] ${msg.sender === "user" ? "self-end items-end" : "self-start items-start"}`}>
                <div className={`p-3 rounded-sm border border-carbon-border  text-sm whitespace-pre-line font-medium ${
                  msg.sender === "user" ? "bg-[#defbe6] text-[#198038] text-white rounded-tr-none" : "bg-carbon-bg text-carbon-text rounded-tl-none"
                }`}>
                  {msg.imageUrl && (
                    <img src={msg.imageUrl} alt="Bot attachment" className="w-full max-h-40 object-cover rounded-sm mb-2 border border-carbon-border " />
                  )}
                  <div>{msg.text}</div>
                </div>
            ...
            {simLoading && (
              <div className="self-start bg-gray-100 p-3 rounded-sm border border-carbon-border text-xs font-semibold animate-pulse">
                البوت يكتب الآن... ✍️
              </div>
            )}
          </div>
          ...
          <form onSubmit={handleSimulate} className="mt-4 flex gap-2">
            <input 
              ...
              className="w-full border border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary flex-1 border border-carbon-border  text-sm font-medium"
              value={simMsg}
            ...
            <button 
              ...
              className="px-4 py-2 text-sm font-medium transition-colors bg-carbon-blue text-white hover:bg-carbon-blueHover text-white px-6 py-3 font-semibold  disabled:opacity-50">
              إرسال
            </button>
          </form>
        </div>
...
export function DeliveryZonesPage() {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="bg-[#fcf4d6] text-[#b47a00] bg-carbon-layer border border-carbon-border p-3 flex flex-row items-center justify-between gap-4 text-white border border-carbon-border ">
        <div className="flex items-center gap-3">
          <MapPin size={20} />
        ...
        <div className="flex items-center gap-2">
          ...
          <select 
            ...
            className="bg-white text-carbon-text font-medium py-1 px-2 text-xs border border-carbon-border outline-none">
            {branches.map(b => (
              <option key={b.id} value={b.id}>{b.name}</option>
            ))}
          </select>
      ...
      <div className="bg-carbon-layer border border-carbon-border p-0 bg-white min-h-[400px] border border-carbon-border ">
        {selectedBranchId && (
          <div className="p-2">
