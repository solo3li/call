...
export function AnalyticsPage() {
 ...
 <button onClick={fetchStats} className="text-carbon-text px-3 py-1 shadow-sm">
 <RefreshCcw size={14} />
 </button>
 <select className="w-full border-carbon-border bg-carbon-bg px-3 py-2 text-sm focus:outline-none focus:border-carbon-blue text-carbon-text placeholder-carbon-textSecondary bg-carbon-layer text-carbon-text font-medium text-xs py-1 h-8" value={timeRange} onChange={(e) => setFilter(e.target.value)}>
 ...
 <div className="bg-carbon-layer border-carbon-border p-4 ">
 ...
 <div className="bg-carbon-layer border-carbon-border p-0 overflow-hidden ">
...
function StatCard({ title, value, icon: Icon, color, trend }: any) {
 return (
 <div className="bg-carbon-layer border-carbon-border p-3 flex flex-col justify-between">
 <div className="flex items-center justify-between mb-2">
 <div className={`${color} p-1.5 border-carbon-border`}>
 <Icon size={14} className="currentColor" />
 </div>
 <span className={`text-[9px] font-semibold px-1.5 py-0.5 border-carbon-border ${trend.startsWith('+') ? 'bg-carbon-success/10 text-carbon-success' : 'bg-carbon-error/10 text-carbon-error'}`}>
 {trend}
 </span>
 </div>
 <div>
